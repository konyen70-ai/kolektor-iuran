...
import { QrCode, Search, Clock, ShieldCheck, RefreshCw, Plus, Trash2, Printer, X, Check, Users, Home, Info, Sparkles, MoreVertical, Edit, Download } from "lucide-react";
...
type ScreenType = "DASHBOARD" | "SCAN" | "MANUAL" | "MANAGE" | "PAYMENT" | "RECEIPT";
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-0 sm:p-5 md:p-8 font-sans selection:bg-blue-200">
      {/* Bingkai HP Android (Mobile-First responsive container) */}
      <div
        className="w-full max-w-md bg-white sm:rounded-[40px] sm:shadow-2xl sm:border-[8px] border-slate-900 overflow-hidden flex flex-col h-screen sm:h-[820px] relative"
      ...
      >
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {(activeScreen === "DASHBOARD" || activeScreen === "SCAN" || activeScreen === "MANUAL" || activeScreen === "MANAGE") && (
            <div className="bg-white border-b border-slate-200 p-4.5 grid grid-cols-2 gap-3 shadow-xs shrink-0">
              ...
              <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-3 flex flex-col justify-center">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Setoran Sesi Ini</span>
                <div className="flex items-baseline gap-1 mt-1">
                  <span className="text-lg font-black text-slate-800 font-mono">{totalTransaksiHariIni}</span>
                  <span className="text-[10px] font-bold text-slate-500">Warga</span>
                </div>
              </div>

              {/* Widget 2: Total Uang Diterima Hari Ini */}
              <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-3 flex flex-col justify-center">
                <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest block">Uang Terkumpul</span>
                <span className="text-sm font-black text-blue-900 font-mono mt-1 truncate">
        ...
        {isLoading && activeScreen === "SCAN" && wargaList.length === 0 ? (
          ...
          <main className={`flex-1 p-5 flex flex-col bg-slate-50 ${activeScreen === "SCAN" ? "overflow-hidden" : "overflow-y-auto"}`}>
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {activeScreen === "DASHBOARD" && (
                  <div className="flex flex-col flex-1 justify-center space-y-5 py-3">
                    {/* Welcome banner */}
                    <div className="bg-blue-50/60 border border-blue-100/80 rounded-xl p-4.5 shadow-xs relative overflow-hidden">
                      <div className="absolute top-0 right-0 w-24 h-24 bg-blue-500/5 rounded-full blur-xl pointer-events-none" />
                      <div className="relative z-10">
                        <span className="text-[9px] font-black tracking-widest text-blue-600 uppercase block">PETUGAS AKTIF</span>
                        <h2 className="text-sm font-black text-slate-900 mt-1 font-sans">Kolektor RT 05</h2>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <button
                        ...
                        className="w-full py-4 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl flex items-center justify-between shadow-md shadow-blue-600/20 group cursor-pointer transition-all duration-200 active:scale-[0.98]"
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-11 h-11 bg-white/10 rounded-lg flex items-center justify-center border border-white/10">
                        ...
                        <div className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-white border border-white/5 text-xs font-bold">
                          →
                        </div>
                      </button>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          ...
                          className="p-4 bg-slate-50/80 hover:bg-slate-100/60 border border-slate-200/60 rounded-xl flex flex-col items-start gap-2.5 text-left transition-all cursor-pointer active:scale-[0.98] shadow-2xs"
                        >
                          <div className="w-9 h-9 bg-white border border-slate-200/50 rounded-lg flex items-center justify-center shadow-3xs">
                            <Search className="w-4 h-4 text-blue-600" />
                          </div>
                          <div>
                            <span className="text-[11px] font-black text-slate-800 block">Cari Manual</span>
                            <span className="text-[9px] text-slate-400 block mt-0.5 leading-tight">Nama / No. Rumah</span>
                          </div>
                        </button>
                        ...
                        >
                          ...
                          <div>
                            <span className="text-[11px] font-black text-slate-800 block">Data Warga</span>
                            <span className="text-[9px] text-slate-400 block mt-0.5 leading-tight">Kelola & Cetak QR</span>
                          </div>
                        </button>
                      </div>
                    </div>
                ...
                {activeScreen === "MANUAL" && (
                  <div className="flex flex-col flex-1 space-y-4">
                    <ManualSearch
                      initialSearchType={manualSearchType}
                    ...
                    <button
                      onClick={() => setActiveScreen("DASHBOARD")}
                      className="w-full py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-xs rounded-2xl transition-all cursor-pointer text-center"
                    >
                      KEMBALI KE BERANDA
                    </button>
                  </div>
                ...
                {activeScreen === "MANAGE" && (
                  <div className="flex flex-col flex-1 space-y-5">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-base font-extrabold text-slate-800">Daftar Warga RT 04</h3>
                        <p className="text-[10px] text-slate-500">Kelola data kepala keluarga dan cetak QR Code</p>
                      </div>

                      <div className="flex gap-2">
                        <button
                          onClick={handleResetDb}
                          className="py-2.5 px-3 bg-white border border-slate-200 text-slate-500 hover:text-slate-800 font-bold text-[11px] rounded-xl flex items-center gap-1 shadow-2xs transition-all cursor-pointer active:scale-95"
                        ...
                        <button
                          ...
                          className="py-2.5 px-3.5 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-[11px] rounded-xl flex items-center gap-1 shadow-sm transition-all cursor-pointer active:scale-95"
                        >
                    ...
                    {showAddForm && (
                      <motion.form
                        ...
                        className="bg-white border border-slate-200 rounded-[24px] p-4.5 space-y-3.5 shadow-xs"
                      >
                        ...
                        <div className="space-y-3">
                          <div className="flex flex-col space-y-1">
                            ...
                            <input
                              ...
                              className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-800 outline-none focus:border-blue-500 focus:bg-white h-11"
                            />
                          </div>
                          ...
                          <div className="flex flex-col space-y-1">
                            ...
                            <input
                              ...
                              className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono font-bold text-slate-800 outline-none focus:border-blue-500 focus:bg-white h-11"
                            />
                            <p className="text-[9px] text-slate-400">
                              *Nomor KK ini akan menjadi basis data pembuatan QR Code warga otomatis.
                            </p>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            <div className="flex flex-col space-y-1">
                              ...
                              <select
                                ...
                                className="w-full px-2 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-800 outline-none focus:border-blue-500 focus:bg-white h-11"
                              >
                        ...
                        <button
                          ...
                          className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs rounded-xl flex items-center justify-center gap-1.5 cursor-pointer shadow-xs"
                        >
                          {isSubmitting ? "Menyimpan..." : "Simpan Data & Buat QR Code"}
                        </button>
                      </motion.form>
                    )}

                    {/* Form Edit Warga */}
                    {showEditForm && editingWarga && (
                      <motion.form
                        initial={{ opacity: 0, height: 0 }}
                        animate={{ opacity: 1, height: "auto" }}
                        onSubmit={handleUpdateWarga}
                        className="bg-white border border-amber-200 bg-amber-50/5 rounded-[24px] p-4.5 space-y-3.5 shadow-xs"
                      >
                        <div className="flex justify-between items-center pb-1.5 border-b border-amber-100">
                          <span className="text-xs font-black text-amber-850 uppercase tracking-wider flex items-center gap-1.5">
                            <Edit className="w-4 h-4 text-amber-600 animate-pulse" />
                            Edit Data Warga: {editingWarga.namaKepalaKeluarga}
                          </span>
                          <button
                            type="button"
                            onClick={() => {
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex flex-col space-y-1">
                            ...
                            />
                            <p className="text-[9px] text-slate-400">
                              *Nomor KK ini akan memperbarui QR Code warga otomatis.
                            </p>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 pt-1">
                          <button
                            ...
                            className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-xs rounded-xl text-center cursor-pointer"
                          >
                            Batal
                          </button>
                          <button
                            type="submit"
                            disabled={isSubmitting}
                            className="w-full py-3 bg-amber-600 hover:bg-amber-700 text-white font-extrabold text-xs rounded-xl flex items-center justify-center gap-1.5 cursor-pointer shadow-xs"
                          >
                    ...
                    <div className="space-y-3 max-h-[480px] overflow-y-auto pr-1">
                      {wargaList.length === 0 ? (
                        <div className="py-12 text-center border-2 border-dashed border-slate-200 rounded-[28px] bg-white text-slate-400">
                          <Users className="w-10 h-10 mx-auto text-slate-300 mb-2" />
                          <h4 className="text-sm font-bold text-slate-700">Database Kosong</h4>
                        ...
                        wargaList.map((w) => (
                          <div
                            key={w.id}
                            className="bg-white border border-slate-200/80 rounded-[24px] p-4 flex flex-col justify-between gap-3 shadow-2xs hover:border-blue-200 transition-all"
                          >
                            {deleteConfirmId === w.id && (
                              <div className="bg-rose-50 border border-rose-100 rounded-2xl p-3 flex flex-col gap-2 mb-1">
                                <div className="flex items-center gap-1.5">
                                  <Trash2 className="w-4 h-4 text-rose-600 shrink-0" />
                                  <p className="text-[11px] font-extrabold text-rose-800 leading-tight">
                                ...
                                <div className="flex items-center gap-2 self-end">
                                  <button
                                    ...
                                    className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-extrabold text-[10px] rounded-lg transition-all cursor-pointer active:scale-95 shadow-2xs"
                                  >
                                    Ya, Hapus
                                  </button>
                                  <button
                                    ...
                                    className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-extrabold text-[10px] rounded-lg transition-all cursor-pointer active:scale-95"
                                  >
                                    Batal
                                  </button>
                                </div>
                            ...
                            <div className="flex justify-between items-start">
                              <div className="space-y-1 min-w-0">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <h4 className="font-extrabold text-slate-800 text-sm truncate">{w.namaKepalaKeluarga}</h4>
                                  <span className="px-1.5 py-0.5 rounded bg-slate-100 text-[8px] font-mono font-bold text-slate-500 uppercase">
                                    {w.id}
                                  </span>
                                </div>
                                <div className="text-[10px] text-slate-500 font-semibold flex items-center gap-1.5 flex-wrap">
                                  ...
                                  <span>Rumah No. {w.nomorRumah}</span>
                                  <span>•</span>
                                ...
                                <div className="text-[10px] font-mono text-slate-400">
                                  No. KK: <span className="font-bold text-slate-600">{w.nomorKk}</span>
                                </div>
                              </div>

                              <span className="text-xs font-black font-mono text-slate-800">
                                {formatRupiah(w.tarifPerBulan)}/bln
                              </span>
                            </div>
                            ...
                            <div className="flex items-center justify-between border-t border-slate-50 pt-3 mt-1.5">
                              ...
                              <button
                                ...
                                className="py-2 px-3 bg-blue-50 hover:bg-blue-100 text-blue-700 font-extrabold text-[10px] rounded-lg transition-colors cursor-pointer flex items-center gap-1 active:scale-95 border border-blue-100"
                              >
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                <button
                                  ...
                                  className="p-2 bg-slate-50 hover:bg-amber-50 text-slate-400 hover:text-amber-600 rounded-lg transition-colors cursor-pointer"
                                ...
                                <button
                                  ...
                                  className="p-2 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-lg transition-colors cursor-pointer border border-rose-100/30"
                                  title="Hapus Warga"
                                >
                    ...
                    <button
                      ...
                      className="w-full py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-xs rounded-2xl transition-all cursor-pointer text-center mt-2"
                    >
                      KEMBALI KE BERANDA
                    </button>
                  </div>
        ...
        <footer className="bg-white border-t border-slate-100 text-[10px] text-slate-500 py-3.5 text-center shrink-0 flex items-center justify-center gap-1.5 select-none">
